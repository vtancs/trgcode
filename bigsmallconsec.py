...
def simulate_sic_bo_games(num_games):
    ...
    consecutive_big_small = 0
    
    for i in range(1, len(outcomes)):
        if (outcomes[i-1] == 'Big' and outcomes[i] == 'Small') or (outcomes[i-1] == 'Small' and outcomes[i] == 'Big'):
            consecutive_big_small += 1
    
    return consecutive_big_small / (num_games - 1)

# Simulate a large number of games
num_games = 100000
probability_consecutive_big_small = simulate_sic_bo_games(num_games)

print(f"Probability of Consecutive Big and Small: {probability_consecutive_big_small:.2%}")
